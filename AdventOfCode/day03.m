...
x=0;y=0;
path1x = zeros(1, length(wire1)+1);
path1y = zeros(1, length(wire1)+1);
path1x(1) = 0;  % Starting point
path1y(1) = 0;
...
%  disp(sprintf('x=%d, y=%d', x, y))
 ...
 % Start over for second path
x=0;y=0;
path2x = zeros(1, length(wire2)+1);
path2y = zeros(1, length(wire2)+1);
path2x(1) = 0;  % Starting point
path2y(1) = 0;
...
% InterX is from: https://www.mathworks.com/matlabcentral/fileexchange/22441-curve-intersections
% Sick the 0,0 leg
P = InterX([path1x(2:length(path1x));path1y(2:length(path1y))], ...
